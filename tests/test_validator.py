...
# ---------------------------------------------------------------------------
# Miner type detection tests
# ---------------------------------------------------------------------------


def _make_kq_resp(has_chunks: bool = False, has_proof: bool = False) -> KnowledgeQuery:
    resp = KnowledgeQuery(query_text="test")
    if has_chunks:
        resp.chunks = [{"text": "chunk", "hash": "a" * 64, "score": 0.9}]
    if has_proof:
        resp.merkle_proof = {"leaf_hash": "a" * 64, "siblings": [], "root": "r" * 64}
    return resp


def _make_nh_resp(has_passage: bool = False) -> NarrativeHop:
    resp = NarrativeHop(destination_node_id="node-1", session_id="test")
    if has_passage:
        resp.narrative_passage = " ".join(["word"] * 150)
        resp.passage_embedding = [1.0] + [0.0] * 767
    return resp


def test_detect_miner_type_domain(
    mock_wallet, mock_subtensor, mock_dendrite, mock_metagraph, graph_store
):
    """KnowledgeQuery with chunks and no NarrativeHop passage -> DOMAIN."""
    from subnet.emissions import MinerType

    v = _make_validator(mock_wallet, mock_subtensor, mock_dendrite, mock_metagraph, graph_store)
    kq = _make_kq_resp(has_chunks=True)
    nh = _make_nh_resp(has_passage=False)

    result = v._detect_miner_type(uid=1, kq_resp=kq, nh_resp=nh)

    assert result == MinerType.DOMAIN
    assert v._miner_types[1] == MinerType.DOMAIN


def test_detect_miner_type_narrative(
    mock_wallet, mock_subtensor, mock_dendrite, mock_metagraph, graph_store
):
    """NarrativeHop with passage and no KnowledgeQuery chunks -> NARRATIVE."""
    from subnet.emissions import MinerType

    v = _make_validator(mock_wallet, mock_subtensor, mock_dendrite, mock_metagraph, graph_store)
    kq = _make_kq_resp(has_chunks=False)
    nh = _make_nh_resp(has_passage=True)

    result = v._detect_miner_type(uid=2, kq_resp=kq, nh_resp=nh)

    assert result == MinerType.NARRATIVE
    assert v._miner_types[2] == MinerType.NARRATIVE


def test_detect_miner_type_unified(
    mock_wallet, mock_subtensor, mock_dendrite, mock_metagraph, graph_store
):
    """Both synapses respond with valid data -> UNIFIED."""
    from subnet.emissions import MinerType
    ...
    result = v._detect_miner_type(uid=3, kq_resp=kq, nh_resp=nh)

    assert result == MinerType.UNIFIED


def test_detect_miner_type_unknown_preserves_existing(
    mock_wallet, mock_subtensor, mock_dendrite, mock_metagraph, graph_store
):
    """When neither synapse responds, the existing cached type is preserved."""
    from subnet.emissions import MinerType

    v = _make_validator(mock_wallet, mock_subtensor, mock_dendrite, mock_metagraph, graph_store)
    v._miner_types[1] = MinerType.DOMAIN  # previously classified
    ...
    assert result == MinerType.DOMAIN  # preserved


def test_detect_miner_type_unknown_new_uid(
    mock_wallet, mock_subtensor, mock_dendrite, mock_metagraph, graph_store
):
    """When neither synapse responds and uid has no cached type -> UNKNOWN."""
    from subnet.emissions import MinerType
    ...
    result = v._detect_miner_type(uid=99, kq_resp=kq, nh_resp=nh)

    assert result == MinerType.UNKNOWN


def test_resync_clears_miner_type_cache_on_hotkey_change(
    mock_wallet, mock_subtensor, mock_dendrite, mock_metagraph, graph_store
):
    """When a hotkey changes, the cached miner type for that UID is evicted."""
    from subnet.emissions import MinerType

    v = _make_validator(mock_wallet, mock_subtensor, mock_dendrite, mock_metagraph, graph_store)
    v._miner_types[1] = MinerType.DOMAIN

    mock_metagraph.hotkeys[1] = "new-miner-hotkey"
    v.resync_metagraph()

    assert 1 not in v._miner_types


async def test_run_epoch_domain_miner_gets_corpus_score(
    mock_wallet, mock_subtensor, mock_dendrite, mock_metagraph, graph_store
):
    """A pure domain miner (no NarrativeHop passage) is classified as DOMAIN
    and receives the corpus score from its merkle proof."""
    from subnet.emissions import MinerType

    # All three miner UIDs (1,2,3) behave as domain miners: respond to
    # KnowledgeQuery with chunks+proof, return nothing for NarrativeHop.
    def domain_kq_handler(synapse, axon_index=0, axon_uid=0):
        if synapse.query_text == "__corpus_challenge__":
            synapse.merkle_proof = {
                "leaf_hash": "a" * 64,
                "siblings": [{"hash": "b" * 64, "position": "right"}],
            ...
            synapse.chunks = [{"text": "chunk text", "hash": "d" * 64, "score": 0.9}]
            synapse.domain_similarity = 0.8
            synapse.node_id = f"node-{axon_uid or axon_index + 1}"
        synapse.agent_uid = axon_uid or axon_index + 1
        return synapse

    def silent_nh_handler(synapse, axon_index=0, axon_uid=0):
        # No passage — domain miner can't serve NarrativeHop
        return synapse

    mock_dendrite.register_handler(KnowledgeQuery, domain_kq_handler)
    mock_dendrite.register_handler(NarrativeHop, silent_nh_handler)

    v = _make_validator(mock_wallet, mock_subtensor, mock_dendrite, mock_metagraph, graph_store)
    await v.run_epoch()

    # All scored miners should be classified as DOMAIN
    for uid in [1, 2, 3]:
        assert v._miner_types.get(uid) == MinerType.DOMAIN, f"UID {uid} not DOMAIN"


async def test_run_epoch_narrative_miner_skips_corpus_gate(
    mock_wallet, mock_subtensor, mock_dendrite, mock_metagraph, graph_store
):
    """A pure narrative miner (no KnowledgeQuery chunks) is classified as NARRATIVE
    and is not penalized by the corpus gate in EmissionCalculator."""
    from subnet.emissions import MinerType

    def silent_kq_handler(synapse, axon_index=0, axon_uid=0):
        # No chunks, no proof — narrative miner can't serve KnowledgeQuery
        return synapse

    def narrative_nh_handler(synapse, axon_index=0, axon_uid=0):
        synapse.narrative_passage = " ".join(["story"] * 200)
        synapse.passage_embedding = [1.0] + [0.0] * 767
        synapse.choice_cards = []
        synapse.agent_uid = axon_uid or axon_index + 1
        return synapse

    mock_dendrite.register_handler(KnowledgeQuery, silent_kq_handler)
    mock_dendrite.register_handler(NarrativeHop, narrative_nh_handler)

    v = _make_validator(mock_wallet, mock_subtensor, mock_dendrite, mock_metagraph, graph_store)
    await v.run_epoch()

    # All scored miners should be classified as NARRATIVE
    for uid in [1, 2, 3]:
        assert v._miner_types.get(uid) == MinerType.NARRATIVE, f"UID {uid} not NARRATIVE"

    # Narrative miners must have received non-trivial rewards (not zero from corpus gate)
    miner_scores = v.scores[1:]
    assert miner_scores.sum() > 0, "Narrative miners should have non-zero scores"
